...
import { LatticeClient } from './latticeClient';
import { ProjectTemplates } from './templates/projectTemplates';

let latticeClient: LatticeClient;
...
export function activate(context: vscode.ExtensionContext) {
    console.log('Lattice AI Blockchain extension is now active!');
    ...
    latticeClient = new LatticeClient(rpcUrl);

    // Initialize providers
    connectionProvider = new LatticeConnectionProvider(latticeClient);
    modelProvider = new LatticeModelProvider(latticeClient);
    networkProvider = new LatticeNetworkProvider(latticeClient);
    ...
    const codeLensProvider = new LatticeCodeLensProvider(latticeClient);
    ...
    vscode.workspace.onDidChangeConfiguration(event => {
        if (event.affectsConfiguration('lattice.rpcUrl')) {
            ...
            if (newRpcUrl) {
                latticeClient.updateRpcUrl(newRpcUrl);
            }
        }
    });
    ...
    latticeClient.onConnectionChange((connected: boolean) => {
        statusBarItem.text = connected ? '$(symbol-misc) Lattice: Connected' : '$(symbol-misc) Lattice: Disconnected';
        statusBarItem.tooltip = connected ? 'Connected to Lattice node' : 'Click to connect to Lattice node';
        statusBarItem.show();
...
function registerCommands(context: vscode.ExtensionContext) {
    ...
    const connectCommand = vscode.commands.registerCommand('lattice.connectNode', async () => {
        ...
        try {
            ...
            await latticeClient.connect();
            vscode.window.showInformationMessage(`Connected to Lattice node at ${rpcUrl}`);
        } catch (error) {
            vscode.window.showErrorMessage(`Failed to connect to Lattice node: ${error}`);
        } finally {
    ...
    const deployCommand = vscode.commands.registerCommand('lattice.deployModel', async (uri?: vscode.Uri) => {
        if (!latticeClient.isConnected) {
            vscode.window.showWarningMessage('Please connect to Lattice node first');
            return;
        }
        ...
        try {
            ...
            await vscode.window.withProgress({
                ...
                const result = await latticeClient.deployModel(
    ...
    const inferenceCommand = vscode.commands.registerCommand('lattice.runInference', async () => {
        ...
        try {
            ...
            const models = await latticeClient.getModels();
            if (models.length === 0) {
            ...
            await vscode.window.withProgress({
                ...
                const result = await latticeClient.runInference(selectedModel.value, parsedInput);
