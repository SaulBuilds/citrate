// citrate/core/execution/src/vm/ai_opcodes.rs
...
impl AIVMExtension {
    ...
    pub fn gas_cost(&self, opcode: AIOpcode) -> u64 {
        match opcode {
            ...
            | AIOpcode::TENSOR_ADD
            | AIOpcode::TENSOR_MUL
            | AIOpcode::TENSOR_MATMUL => self.gas_costs.tensor_op,
    ...
    fn execute_load_model(&mut self) -> Result<(), ExecutionError> {
        ...
        let model_id = self.stack.pop().ok_or(ExecutionError::StackUnderflow)?;
    ...
    fn execute_unload_model(&mut self) -> Result<(), ExecutionError> {
        ...
        // Pop model ID from stack
        let _model_id = self.stack.pop().ok_or(ExecutionError::StackUnderflow)?;
    ...
    fn execute_model(&mut self) -> Result<(), ExecutionError> {
        ...
        let input_id = self.stack.pop().ok_or(ExecutionError::StackUnderflow)?;
        let model_id = self.stack.pop().ok_or(ExecutionError::StackUnderflow)?;
    ...
    fn execute_train_model(&mut self) -> Result<(), ExecutionError> {
        ...
        let dataset_id = self.stack.pop().ok_or(ExecutionError::StackUnderflow)?;
        let model_id = self.stack.pop().ok_or(ExecutionError::StackUnderflow)?;
    ...
    fn execute_tensor_new(&mut self) -> Result<(), ExecutionError> {
        ...
        let _dims = self.stack.pop().ok_or(ExecutionError::StackUnderflow)?;
        let height = self
            .stack
            .pop()
    ...
    fn execute_tensor_add(&mut self) -> Result<(), ExecutionError> {
        ...
        let b_id = self.stack.pop().ok_or(ExecutionError::StackUnderflow)?;
        let a_id = self.stack.pop().ok_or(ExecutionError::StackUnderflow)?;
    ...
    fn execute_verify_proof(&mut self) -> Result<(), ExecutionError> {
        ...
        let _proof_id = self.stack.pop().ok_or(ExecutionError::StackUnderflow)?;
