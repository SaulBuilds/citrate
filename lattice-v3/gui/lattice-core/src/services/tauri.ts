...
const loadWebAccounts = () => {
  const stored = localStorage.getItem('lattice_web_accounts');
  if (stored) {
    try {
...
const saveWebAccounts = () => {
  ...
  localStorage.setItem('lattice_web_accounts', JSON.stringify(serialized));
};
...
const webImplementations = {
  ...
  start_node: async () => {
    ...
    if (connected) {
      ...
      throw new Error('Cannot connect to node at http://127.0.0.1:8545. Make sure the Lattice node is running.');
    }
  },
  ...
  get_dag_data: async (args: { limit: number, startHeight?: number }) => {
    try {
      ...
      try {
        const data = await (rpcClient as any).sendRequest?.('lattice_getDagData', [args.limit, args.startHeight])
          .catch(() => null);
  ...
  get_model_info: async (args: { modelId: string }) => {
    const models = await rpcClient.getModelRegistry();
    return models.find((m: any) => m.id === args.modelId) || null;
  },
