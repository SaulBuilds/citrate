...
import { CitrateService } from './services/CitrateService';
...
function App() {
  ...
  const [latticeService, setCitrateService] = useState(null);
  const [connectionStatus, setConnectionStatus] = useState('disconnected');
  const [notifications, setNotifications] = useState([]);
  const [isRunning, setIsRunning] = useState(false);

  useEffect(() => {
    // Initialize Citrate service
    const service = new CitrateService();
    setCitrateService(service);
    ...
    const checkConnection = async () => {
      try {
        ...
        addNotification('Failed to connect to Citrate node', 'error');
      }
    };
  ...
  const getConnectionText = () => {
    switch (connectionStatus) {
      case 'connected': return 'Connected to Citrate Node';
      case 'disconnected': return 'Disconnected from Citrate Node';
