...
use citrate_execution::{state::StateDB, Executor};
use citrate_network::peer::{Direction as PeerDirection, PeerId, PeerState as NetPeerState};
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        if config.enable_network {
            ...
            tokio::spawn(async move {
                use citrate_network::{protocol::PeerAddress, NetworkMessage};
                                    ...
                                    if let Ok(addr) = pa.addr.parse() {
                                        let _ = pm_for_listener
                                            ...
                                            .connect_bootnode_real(
                                                Some(citrate_network::peer::PeerId::new(pa.id)),
                                                addr,
                                                network_id,
                                                genesis_hash,
                                                head_height,
...
fn parse_bootnode(s: &str) -> Option<(citrate_network::peer::PeerId, SocketAddr)> {
    ...
    let peer_id = peer_part
        .map(|p| citrate_network::peer::PeerId::new(p.to_string()))
        .unwrap_or_else(citrate_network::peer::PeerId::random);
    Some((peer_id, addr))
}
