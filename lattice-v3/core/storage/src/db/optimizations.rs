use rocksdb::{Options, BlockBasedOptions, Cache, SliceTransform};
...
impl DbOptimizations {
    ...
    pub fn apply_cf_optimizations(cf_name: &str, opts: Options) -> Options {
        match cf_name {
            "headers" | "tips" | "mergeset" | "blue_score" => {
                Self::optimize_metadata_cf(opts)
            }
            "transactions" => {
                Self::optimize_transactions_cf(opts)
            }
            "state" | "accounts" | "storage" | "code" => {
                Self::optimize_state_cf(opts)
            }
            "receipts" | "logs" => {
                Self::optimize_receipts_cf(opts)
            }
            _ => opts,
        }
    }
