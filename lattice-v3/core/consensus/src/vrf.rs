...
impl VrfProposerSelector {
    ...
    pub async fn is_eligible_proposer(
        ...
        let validator = validators
            .get(pubkey)
            .ok_or(VrfError::ValidatorNotFound)?;
...
impl LeaderElection {
    ...
    pub async fn verify_leader(
        ...
        // Verify VRF proof
        if !self.vrf_selector.verify_vrf_proof(pubkey, proof, previous_vrf, slot)? {
            return Ok(false);
        }
        
        // Check eligibility
        self.vrf_selector.is_eligible_proposer(pubkey, &proof.output, slot).await
    }
}
...
mod tests {
    ...
    async fn test_vrf_proof_generation() {
        ...
        let proof = selector.generate_vrf_proof(&secret_key, &previous_vrf, slot).unwrap();
    ...
    async fn test_leader_election() {
        ...
        let previous_vrf = Hash::new([0; 32]);
        let leader = leader_election.elect_leader(50, &previous_vrf).await.unwrap();
