// lattice-v3/core/api/src/enhanced_tx_decoder.rs
...
impl EnhancedTransactionDecoder {
    ...
    pub fn decode_transaction(&self, tx_bytes: &[u8]) -> Result<DecodedTransaction, TransactionDecoderError> {
        ...
        // Try Lattice native format first
        ...
        let result = match tx_type {
            ...
            TransactionType::LatticeNative => unreachable!("Already handled above"),
        };
        ...
        match &result {
            Ok(decoded) => {
                ...
                let tx_type_str = match decoded.tx_type {
                    ...
                    TransactionType::LatticeNative => "lattice_native",
                };
    ...
    fn decode_eip2930_transaction(&self, tx_bytes: &[u8]) -> Result<DecodedTransaction, TransactionDecoderError> {
        ...
        // Convert to Lattice transaction
    ...
    /// Handle Lattice native transaction
    fn handle_lattice_native_transaction(&self, mut tx: Transaction) -> Result<DecodedTransaction, TransactionDecoderError> {
        ...
        let mut stats = self.stats.lock().unwrap();
        stats.record_successful_decode(self.config.default_chain_id, "lattice_native");

        let gas_price = tx.gas_price; // Extract before moving tx
        Ok(DecodedTransaction {
            transaction: tx,
            tx_type: TransactionType::LatticeNative,
