...
# Complete wallet and rewards setup for Lattice V3
...
echo -e "${CYAN}╔══════════════════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║           LATTICE V3 WALLET & REWARDS SETUP                         ║${NC}"
echo -e "${CYAN}╚══════════════════════════════════════════════════════════════════════╝${NC}"
...
rm -rf .lattice-testnet 2>/dev/null || true
rm -rf .lattice-devnet 2>/dev/null || true
...
RUST_LOG=info ./target/release/lattice --config testnet-config.toml --data-dir .lattice-testnet --mine > /tmp/lattice_core.log 2>&1 &
CORE_PID=$!
...
for i in {1..120}; do
    if grep -q "Setting up node components" /tmp/lattice_gui.log 2>/dev/null; then
...
echo -e "  ${CYAN}tail -f /tmp/lattice_core.log | grep -E \"Produced block|Minted.*validator\"${NC}"
