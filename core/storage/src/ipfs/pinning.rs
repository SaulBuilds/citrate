...
//! rewards they have accrued so far. The accounting mirrors the
//! incentivisation logic that will eventually be enforced on-chain
//! by the IPFS incentives contract.
...
impl PinningManager {
    /// Create a fresh manager with no state.
    pub fn new() -> Self {
        Self {
            records: HashMap::new(),
        }
    ...
    /// Record a pinning report and return the computed reward.
    ///
    /// * `cid` - Content identifier being pinned.
    /// * `pinner_id` - Identifier of the node/provider reporting the pin.
    /// * `metadata` - Model metadata associated with the CID.
    /// * `pinned_bytes` - Number of bytes pinned in this report.
    pub fn record_pin(
        &mut self,
        cid: Cid,
        pinner_id: String,
    ...
    /// Provide a summary of the pinning status for the supplied CID.
    ...
    /// Estimate the incentive reward for keeping the supplied metadata pinned
    /// for `duration_hours`.
    ...
    /// Internal helper used when a pin report is filed for a specific byte
    /// amount (as opposed to a duration estimate).
...
mod tests {
    ...
    fn reward_for_bytes_scales_with_model_type() {
        let language_reward =
            PinningManager::reward_for_bytes(1_073_741_824, &ModelType::Language); // 1GB
        let vision_reward =
    ...
    fn record_pin_tracks_replicas_and_rewards() {
        ...
        let metadata = sample_metadata(ModelType::Vision, 1_500_000_000); // ~1.4GB
    ...
    fn reward_for_duration_matches_expected_multiplier() {
        let metadata = sample_metadata(ModelType::Multimodal, 2_147_483_648); // 2GB
        let reward = PinningManager::reward_for_duration(&metadata, 48); // 2 days
        // Multimodal multiplier = 4, 2GB * 2 days = 4 -> reward 16
        assert_eq!(reward, 16);
    }
}
