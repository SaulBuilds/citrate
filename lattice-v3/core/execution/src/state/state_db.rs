...
impl StateDB {
    ...
    /// Register model
    pub fn register_model(&self, model_id: ModelId, model: ModelState) -> Result<(), ExecutionError> {
    ...
    /// Update training job
    pub fn update_training_job(&self, job_id: JobId, job: TrainingJob) -> Result<(), ExecutionError> {
        if !self.training_jobs.contains_key(&job_id) {
    ...
    pub fn snapshot(&self) -> StateSnapshot {
        StateSnapshot {
            ...
            storage_tries: self.storage_tries
                .iter()
