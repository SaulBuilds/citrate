"""
Unit tests for Lattice SDK client
"""
...
from lattice_sdk import LatticeClient, ModelConfig, ModelType, AccessType
from lattice_sdk.errors import LatticeError, ModelNotFoundError
from lattice_sdk.crypto import KeyManager


class TestLatticeClient:
    """Test cases for LatticeClient"""
    ...
    def test_rpc_call_success(self, mock_post):
        ...
        client = LatticeClient(self.mock_rpc_url)
    ...
    def test_rpc_call_error(self, mock_post):
        ...
        with pytest.raises(LatticeError, match="RPC error: Invalid params"):
    ...
    def test_client_initialization_with_key(self):
        ...
        client = LatticeClient(self.mock_rpc_url, self.mock_private_key)
    ...
    @patch.object(LatticeClient, '_rpc_call')
    @patch.object(LatticeClient, '_upload_to_ipfs')
    @patch.object(LatticeClient, '_send_transaction')
    @patch.object(LatticeClient, '_wait_for_receipt')
    @patch.object(LatticeClient, '_extract_model_id_from_receipt')
    ...
    def test_deploy_model_without_key(self):
        ...
        client = LatticeClient(self.mock_rpc_url)  # No private key
        config = ModelConfig()

        with pytest.raises(LatticeError, match="Private key required"):
    ...
    def test_deploy_model_nonexistent_file(self):
        ...
        with pytest.raises(LatticeError, match="Model file not found"):
            client.deploy_model("nonexistent.json", config)

    @patch.object(LatticeClient, '_rpc_call')
    @patch.object(LatticeClient, '_send_transaction')
    @patch.object(LatticeClient, '_wait_for_receipt')
    @patch.object(LatticeClient, '_extract_inference_output')
...
class TestErrorHandling:
    ...
    def test_network_error(self, mock_post):
        ...
        with pytest.raises(LatticeError, match="Network error"):
    ...
    def test_invalid_json_response(self, mock_post):
        ...
        with pytest.raises(LatticeError, match="Invalid JSON response"):
            client._rpc_call("test_method")