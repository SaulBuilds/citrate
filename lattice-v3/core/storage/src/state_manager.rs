...
impl StateManager {
    ...
    pub fn register_model(&self, model_id: ModelId, model_state: ModelState, weight_cid: String) -> Result<()> {
        ...
        self.ai_state.write().register_model(model_id, model_state, weight_cid);
    ...
    pub fn update_model_weights(&self, model_id: ModelId, new_cid: String, new_version: u32) -> Result<()> {
        self.ai_state.write().update_model_weights(model_id, new_cid.clone(), new_version);
        
        // Persist weight CID
        let key = format!("weights:{}", hex::encode(model_id.0.as_bytes()));
        self.db.put_cf("state", key.as_bytes(), new_cid.as_bytes())?;
        
        info!("Updated model {:?} weights to version {}", model_id, new_version);
        Ok(())
    }
    
    /// Add training job
    pub fn add_training_job(&self, job_id: JobId, job: TrainingJob) -> Result<()> {
        self.ai_state.write().training_jobs.insert(job_id, job.clone());
    ...
    pub fn add_lora_adapter(&self, adapter: crate::state::LoRAAdapter) -> Result<()> {
        ...
        info!("Added LoRA adapter {:?} for model {:?}", adapter.adapter_id, adapter.base_model);
        Ok(())
    }
    ...
    pub fn prune_inference_cache(&self, max_age: u64) {
        ...
        self.ai_state.write().prune_inference_cache(max_age, current_time);
        info!("Pruned inference cache entries older than {} seconds", max_age);
    }
...
mod tests {
    ...
    async fn test_unified_state_root() {
        ...
        state_manager.register_model(model_id, model_state, "QmTest".to_string()).unwrap();
