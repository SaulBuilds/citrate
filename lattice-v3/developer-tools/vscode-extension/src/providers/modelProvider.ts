...
import { LatticeClient, LatticeModel } from '../latticeClient';
...
export class LatticeModelProvider implements vscode.TreeDataProvider<ModelItem> {
    ...
    constructor(private latticeClient: LatticeClient) {}
    ...
    async getChildren(element?: ModelItem): Promise<ModelItem[]> {
        if (!this.latticeClient.isConnected) {
            return [];
        }
        ...
        if (!element) {
            ...
            try {
                const models = await this.latticeClient.getModels();
            ...
            return [
                ...
                new ModelItem(`Owner: ${this.latticeClient.formatAddress(model.owner || '')}`, undefined, vscode.TreeItemCollapsibleState.None),
                new ModelItem(`Price: ${model.price ? this.latticeClient.formatEther(model.price) : '0'} ETH`, undefined, vscode.TreeItemCollapsibleState.None),
                new ModelItem(`Inferences: ${model.totalInferences || 0}`, undefined, vscode.TreeItemCollapsibleState.None),
                new ModelItem(`Revenue: ${model.totalRevenue ? this.latticeClient.formatEther(model.totalRevenue) : '0'} ETH`, undefined, vscode.TreeItemCollapsibleState.None),
            ];
        }
