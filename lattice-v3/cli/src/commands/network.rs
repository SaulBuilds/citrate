...
async fn get_status(config: &Config) -> Result<()> {
    ...
    println!("Syncing: {}", if syncing { "Yes".yellow() } else { "No (synced)".green() });
...
async fn get_block(config: &Config, block: &str) -> Result<()> {
    ...
    let block_param = if block == "latest" {
        ...
        format!("0x{:x}", block.parse::<u64>().context("Invalid block number")?)
    };
    ...
    if let Some(block) = result["result"].as_object() {
        ...
        println!("Number: {}", u64::from_str_radix(
            block["number"].as_str().unwrap_or("0x0").trim_start_matches("0x"), 16
        ).unwrap_or(0));
        println!("Hash: {}", block["hash"].as_str().unwrap_or("N/A").cyan());
        println!("Parent: {}", block["parentHash"].as_str().unwrap_or("N/A"));
        println!("Timestamp: {}", u64::from_str_radix(
            block["timestamp"].as_str().unwrap_or("0x0").trim_start_matches("0x"), 16
        ).unwrap_or(0));
...
async fn get_transaction(config: &Config, tx_hash: &str) -> Result<()> {
    ...
    if let Some(tx) = result["result"].as_object() {
        ...
        println!("Nonce: {}", u64::from_str_radix(
            tx["nonce"].as_str().unwrap_or("0x0").trim_start_matches("0x"), 16
        ).unwrap_or(0));
        
        println!("Gas Price: {} gwei", u64::from_str_radix(
            tx["gasPrice"].as_str().unwrap_or("0x0").trim_start_matches("0x"), 16
        ).unwrap_or(0) / 1_000_000_000);
        
        println!("Gas Limit: {}", u64::from_str_radix(
            tx["gas"].as_str().unwrap_or("0x0").trim_start_matches("0x"), 16
        ).unwrap_or(0));
        
        if let Some(block_number) = tx["blockNumber"].as_str() {
            println!("Block: {}", u64::from_str_radix(
                block_number.trim_start_matches("0x"), 16
            ).unwrap_or(0));
        } else {
        ...
        if tx["blockNumber"].is_string() {
            ...
            if let Some(receipt) = receipt_result["result"].as_object() {
                ...
                println!("  Gas Used: {}", u64::from_str_radix(
                    receipt["gasUsed"].as_str().unwrap_or("0x0").trim_start_matches("0x"), 16
                ).unwrap_or(0));
...
async fn get_sync_status(config: &Config) -> Result<()> {
    ...
    if let Some(sync) = result["result"].as_object() {
        ...
        if let (Some(current), Some(highest)) = (
            sync["currentBlock"].as_str(),
            sync["highestBlock"].as_str(),
        ) {
            let current = u64::from_str_radix(current.trim_start_matches("0x"), 16).unwrap_or(0);
...
async fn get_dag_stats(config: &Config) -> Result<()> {
    ...
    if let Some(stats) = result["result"].as_object() {
        ...
        println!("Total Blocks: {}", stats["totalBlocks"].as_u64().unwrap_or(0));
        ...
        println!("Max Blue Score: {}", stats["maxBlueScore"].as_u64().unwrap_or(0));
        ...
        if error["message"].as_str().unwrap_or("").contains("not found") {
            println!("{}", "DAG statistics not available (custom RPC method)".yellow());
