#!/usr/bin/env python3
"""
Import and deploy AI models to Lattice blockchain.
...
class ModelImporter:
    """Import and deploy models to Lattice."""
    ...
    def __init__(self, rpc_url: str = DEFAULT_RPC, ipfs_api: str = DEFAULT_IPFS):
        ...
        try:
            ...
            print(f"Connected to Lattice at {rpc_url}")
        except Exception as e:
            print(f"Failed to connect to Lattice node: {e}")
            print("Make sure Lattice node is running")
            sys.exit(1)
    ...
    def _register_on_chain(self, metadata: Dict[str, Any]) -> str:
        """Register model on Lattice blockchain."""
        # Use Lattice RPC to deploy model
        payload = {
            "jsonrpc": "2.0",
...
def main():
    parser = argparse.ArgumentParser(
        description="Import and deploy AI models to Lattice blockchain"
    )
