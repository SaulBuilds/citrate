...
impl MempoolApi {
    ...
    pub async fn get_transaction(&self, hash: Hash) -> Result<Option<TransactionResponse>, ApiError> {
        let tx = self.mempool.get_transaction(&hash).await;
        Ok(tx.map(Into::into))
    }
    
    /// List pending transactions
    pub async fn get_pending(&self, limit: Option<usize>) -> Result<Vec<TransactionResponse>, ApiError> {
