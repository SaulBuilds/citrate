/**
 * WebSocket client for real-time Lattice communication
 */

import EventEmitter from 'eventemitter3';
import { WebSocketEvent } from '../types/Client';
import { InferenceResult } from '../types/Inference';
import { LatticeError } from '../errors/LatticeError';
...
export class WebSocketClient extends EventEmitter {
  ...
  async connect(): Promise<void> {
    ...
    if (this.isConnecting) {
      throw new LatticeError('Connection already in progress');
    }
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.ws.onerror = (error) => {
          this.isConnecting = false;
          const latticeError = new LatticeError('WebSocket error');
          this.emit('error', {
            type: 'error',
            data: latticeError,
            timestamp: Date.now()
        ...
        setTimeout(() => {
          if (this.isConnecting) {
            this.isConnecting = false;
            reject(new LatticeError('Connection timeout'));
          }
        }, this.config.timeout);

      } catch (error) {
        this.isConnecting = false;
        reject(error instanceof Error ? error : new LatticeError('Connection failed'));
      }
    });
  }
  ...
  disconnect(): void {
    ...
    for (const [id, request] of this.pendingRequests) {
      ...
      request.reject(new LatticeError('Connection closed'));
    }
  ...
  async sendMessage<T = any>(method: string, params: any[] = []): Promise<T> {
    if (!this.isConnected()) {
      throw new LatticeError('WebSocket not connected');
    }
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        reject(new LatticeError('Request timeout'));
      }, this.config.timeout);
  ...
  async startStreamingInference(options: StreamingInferenceOptions): Promise<void> {
    ...
    const handleError = (data: any) => {
      if (data.streamId === streamId) {
        ...
        if (options.onError) {
          options.onError(new LatticeError(data.error));
        }
      }
    };
    ...
    // Start streaming inference
    await this.sendMessage('lattice_startStreamingInference', [{
      streamId,
  ...
  async subscribeToModel(modelId: string): Promise<void> {
    await this.sendMessage('lattice_subscribeModel', [modelId]);
  }
  ...
  async unsubscribeFromModel(modelId: string): Promise<void> {
    await this.sendMessage('lattice_unsubscribeModel', [modelId]);
  }
  ...
  async subscribeToMarketplace(): Promise<void> {
    await this.sendMessage('lattice_subscribeMarketplace', []);
  }
  ...
  private handleMessage(data: string): void {
    try {
      ...
      if (message.id && this.pendingRequests.has(message.id)) {
        ...
        if (message.error) {
          request.reject(new LatticeError(message.error.message));
        } else {
      ...
      this.emit('error', {
        type: 'error',
        data: new LatticeError('Failed to parse WebSocket message'),
        timestamp: Date.now()
      });
    }
  }
  ...
  private handleNotification(method: string, params: any): void {
    switch (method) {
      case 'lattice_inferencePartial':
        this.emit('inferencePartial', params);
        break;

      case 'lattice_inferenceComplete':
        this.emit('inferenceComplete', params);
        break;

      case 'lattice_inferenceError':
        this.emit('inferenceError', params);
        break;

      case 'lattice_modelDeployed':
        this.emit('modelDeployed', params);
        break;

      case 'lattice_modelUpdated':
        this.emit('modelUpdated', params);
        break;

      case 'lattice_marketplaceSale':
