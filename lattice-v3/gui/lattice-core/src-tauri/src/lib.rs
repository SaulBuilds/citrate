...
use node::TxOverview;
use node::TxActivity;
use wallet::{WalletManager, Account, TransactionRequest};
use lattice_network::NetworkMessage;
use lattice_sequencer::mempool::TxClass;
use dag::{DAGManager, DAGData, BlockDetails, TipInfo};
use models::{
    ModelManager, ModelInfo, ModelDeployment, 
...
async fn get_node_status(state: State<'_, AppState>) -> Result<NodeStatus, String> {
    state.node_manager.get_status().await
        .map_err(|e| e.to_string())
}
...
#[tauri::command]
async fn update_node_config(state: State<'_, AppState>, config: NodeConfig) -> Result<String, String> {
...
async fn add_bootnode(state: State<'_, AppState>, entry: String) -> Result<String, String> {
    state.node_manager.add_bootnode_entry(&entry).await
...
async fn remove_bootnode(state: State<'_, AppState>, entry: String) -> Result<String, String> {
    state.node_manager.remove_bootnode_entry(&entry).await
...
async fn get_account_activity(
    ...
    state.node_manager.get_account_activity(&address, bw, lim).await
        .map_err(|e| e.to_string())
}

#[tauri::command]
async fn get_tx_overview(state: State<'_, AppState>) -> Result<TxOverview, String> {
    state.node_manager.get_tx_overview().await.map_err(|e| e.to_string())
}

#[tauri::command]
async fn get_mempool_pending(state: State<'_, AppState>, limit: Option<usize>) -> Result<Vec<PendingTx>, String> {
    state.node_manager.get_mempool_pending(limit.unwrap_or(50)).await.map_err(|e| e.to_string())
}
...
async fn get_address_observed_balance(
    ...
    state.node_manager.get_observed_balance(&address, block_window.unwrap_or(256)).await
        .map_err(|e| e.to_string())
}
...
async fn join_testnet(state: State<'_, AppState>, args: JoinTestnetArgs) -> Result<String, String> {
    ...
    if let Some(bn) = args.bootnodes { cfg.bootnodes = bn; }
    // Resolve a safe dataDir: avoid src-tauri (dev watcher) and relative paths
    let mut desired_data_dir = args.data_dir.clone().unwrap_or_else(|| cfg.data_dir.clone());
    ...
    state.node_manager.update_config(cfg.clone()).await.map_err(|e| e.to_string())?;
    ...
    state.node_manager.start().await.map_err(|e| e.to_string())?;
    ...
    if cfg_after.enable_network && !cfg_after.bootnodes.is_empty() {
        let ok = state.node_manager.connect_bootnodes_now().await.map_err(|e| e.to_string())?;
...
async fn connect_to_external_testnet(
    ...
    *state.external_rpc.write().await = Some(client);
    ...
    info!("Connected to external testnet - Chain ID: {}, Block: {}", chain_id, block_number);
    
    Ok(format!("Connected to testnet - Chain ID: {}, Current Block: {}", chain_id, block_number))
}
...
async fn switch_to_testnet(state: State<'_, AppState>) -> Result<String, String> {
    ...
    // Update the config
    state.node_manager.update_config(config.clone()).await
        .map_err(|e| e.to_string())?;
    ...
    info!("GUI switched to testnet mode - Chain ID: {}, P2P: {}", 
          config.mempool.chain_id, config.p2p_port);
    
    Ok(format!("Connected to testnet - Chain ID: {}, P2P Port: {}", 
               config.mempool.chain_id, config.p2p_port))
}
    ...
    if let Ok(s) = UdpSocket::bind("0.0.0.0:0") {
        if s.connect("1.1.1.1:80").is_ok() {
            if let Ok(addr) = s.local_addr() {
                if let IpAddr::V4(ipv4) = addr.ip() { return Some(ipv4.to_string()); }
            }
        }
    }
    None
...
async fn auto_add_bootnodes(state: State<'_, AppState>) -> Result<Vec<String>, String> {
    ...
    let mut cfg = state.node_manager.get_config().await;
    for e in &entries {
        if !cfg.bootnodes.contains(e) { cfg.bootnodes.push(e.clone()); }
    }
    state.node_manager.update_config(cfg.clone()).await.map_err(|e| e.to_string())?;
...
async fn create_account(
    ...
    state.wallet_manager.create_account(label, &password).await
        .map_err(|e| e.to_string())
}
...
async fn create_account_extended(
    ...
    state.wallet_manager.create_account_with_credentials(label, &password).await
        .map_err(|e| e.to_string())
}
...
async fn import_account(
    ...
    state.wallet_manager.import_account(&private_key, label, &password).await
        .map_err(|e| e.to_string())
}
...
async fn import_account_from_mnemonic(
    ...
    state.wallet_manager.import_account_from_mnemonic(&mnemonic, label, &password).await
        .map_err(|e| e.to_string())
}
...
#[tauri::command]
async fn get_account(state: State<'_, AppState>, address: String) -> Result<Option<Account>, String> {
...
async fn send_transaction(
    ...
    if let Some(rpc_client) = external_rpc.as_ref() {
        ...
        let tx = state.wallet_manager.create_signed_transaction(request.clone(), &password).await
            .map_err(|e| e.to_string())?;
        ...
        let tx_hash = rpc_client.send_raw_transaction(&tx_hex).await
        ...
        if let Some(mempool) = state.node_manager.get_mempool().await {
            let _ = mempool.read().await.add_transaction(tx.clone(), TxClass::Standard).await;
        }
        // Broadcast to peers
        let _ = state.node_manager.broadcast_network(NetworkMessage::NewTransaction { transaction: tx }).await;
        Ok(tx_hash_hex)
    }
}
...
async fn sign_message(
    ...
    state.wallet_manager.sign_message(message.as_bytes(), &address, &password).await
        .map_err(|e| e.to_string())
}
...
async fn verify_signature(
    ...
    state.wallet_manager.verify_signature(message.as_bytes(), &signature, &address).await
        .map_err(|e| e.to_string())
}
...
async fn export_private_key(
    ...
    state.wallet_manager.export_private_key(&address, &password).await
        .map_err(|e| e.to_string())
}
...
async fn update_balance(
    ...
    state.wallet_manager.update_balance(&address, balance_u128).await
        .map_err(|e| e.to_string())
}
...
async fn get_dag_data(
    ...
    if let Some(dag_manager) = dag_manager_opt.as_ref() {
        dag_manager.get_dag_data(limit, start_height).await
            .map_err(|e| e.to_string())
    } else {
...
async fn get_block_details(
    ...
    let dag_manager_opt = state.dag_manager.read().await;
    if let Some(dag_manager) = dag_manager_opt.as_ref() {
        dag_manager.get_block_details(&hash).await
            .map_err(|e| e.to_string())
    } else {
...
async fn get_blue_set(
    ...
    let dag_manager_opt = state.dag_manager.read().await;
    if let Some(dag_manager) = dag_manager_opt.as_ref() {
        dag_manager.get_blue_set(&block_hash).await
            .map_err(|e| e.to_string())
    } else {
        Err("Node is not running. Please start the node first.".to_string())
    }
...
async fn get_current_tips(state: State<'_, AppState>) -> Result<Vec<TipInfo>, String> {
    let dag_manager_opt = state.dag_manager.read().await;
    if let Some(dag_manager) = dag_manager_opt.as_ref() {
        dag_manager.get_current_tips().await
            .map_err(|e| e.to_string())
    } else {
        Ok(vec![])
    }
...
async fn calculate_blue_score(
    ...
    let dag_manager_opt = state.dag_manager.read().await;
    if let Some(dag_manager) = dag_manager_opt.as_ref() {
        dag_manager.calculate_blue_score(&block_hash).await
            .map_err(|e| e.to_string())
    } else {
        Err("Node is not running. Please start the node first.".to_string())
    }
...
async fn get_block_path(
    ...
    let dag_manager_opt = state.dag_manager.read().await;
    if let Some(dag_manager) = dag_manager_opt.as_ref() {
        dag_manager.get_block_path(&block_hash).await
            .map_err(|e| e.to_string())
    } else {
        Err("Node is not running. Please start the node first.".to_string())
    }
...
async fn deploy_model(
    ...
    state.model_manager.deploy_model(deployment).await
        .map_err(|e| e.to_string())
}
...
async fn run_inference(
    ...
    state.model_manager.request_inference(request).await
        .map_err(|e| e.to_string())
}
...
async fn start_training(
    ...
    state.model_manager.start_training(job).await
        .map_err(|e| e.to_string())
}
...
async fn get_job_status(
    ...
    state.model_manager.get_job_status(&job_id).await
        .map_err(|e| e.to_string())
}

#[tauri::command]
async fn get_deployments(state: State<'_, AppState>) -> Result<Vec<ModelDeployment>, String> {
    state.model_manager.get_deployments().await
        .map_err(|e| e.to_string())
}
...
async fn setup_node_components(app_handle: tauri::AppHandle) {
    ...
    if let Ok(addr) = std::env::var("LATTICE_REWARD_ADDRESS") { use_reward_address = Some(addr); }
    // Otherwise prefer config reward address
    if use_reward_address.is_none() {
        let cfg = state.node_manager.get_config().await;
        if let Some(addr) = cfg.reward_address.clone() { use_reward_address = Some(addr); }
    }
    ...
    if !wallets.is_empty() {
        ...
        if use_reward_address.is_none() { use_reward_address = Some(reward_address); }
    } else {
