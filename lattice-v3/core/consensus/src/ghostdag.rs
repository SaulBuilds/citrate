use crate::types::{Block, BlueSet, Hash, GhostDagParams, DagRelation};
...
impl GhostDag {
    ...
    pub async fn calculate_blue_set(&self, block: &Block) -> Result<BlueSet, GhostDagError> {
        ...
        let blue_merge_parents = self.calculate_blue_merge_parents(
            block,
        ...
        info!("Calculated blue set for block {}: score={}", block.hash(), blue_set.score);
        Ok(blue_set)
    }
    ...
    async fn calculate_blue_merge_parents(
        ...
        for merge_parent in &block.header.merge_parent_hashes {
            if self.is_blue_candidate(merge_parent, selected_parent_blue, &blue_parents).await? {
    ...
    async fn is_blue_candidate(
        ...
        let anticone_size = self.count_blue_anticone(
            candidate,
            selected_parent_blue,
    ...
    async fn is_ancestor_of(&self, ancestor: &Hash, descendant: &Hash) -> Result<bool, GhostDagError> {
    ...
    pub async fn add_block(&self, block: &Block) -> Result<(), GhostDagError> {
        ...
        info!("Added block {} to DAG with blue score {}", block.hash(), blue_set.score);
        Ok(())
    }
...
mod tests {
    ...
    async fn test_genesis_block_blue_set() {
        ...
        let genesis = create_test_block_with_parents(
            [0; 32],
            Hash::default(),
            vec![],
            0
    ...
    async fn test_add_block() {
        ...
        ghostdag.blue_cache.write().await.insert(genesis.hash(), blue_set);
        ...
        ghostdag.relations.write().await.insert(genesis.hash(), genesis_relation);
        
        // Add child block
        let block1 = create_test_block_with_parents(
            [1; 32],
            genesis.hash(),
            vec![],
            1
    ...
    async fn test_tip_selection() {
        ...
        ghostdag.relations.write().await.insert(genesis.hash(), genesis_relation);
        ghostdag.blue_cache.write().await.insert(genesis.hash(), genesis_blue);
