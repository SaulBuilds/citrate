...
# Scaffold and run 5 local citrate-node instances bound to the machine's private IP.
#
# Usage:
...
SCAFFOLD_DIR="$ROOT_DIR/.citrate-multi"
BIN="$ROOT_DIR/target/release/citrate-node"
...
case "$ACTION" in
  up)
    echo "Building citrate-node (release)…"
    (cd "$ROOT_DIR" && cargo build --release -p citrate-node)
