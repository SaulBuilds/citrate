/**
 * React hooks for Lattice SDK
 * Optional React integration for web applications
 */

import { useState, useEffect, useCallback } from 'react';
import { LatticeClient, LatticeClientConfig } from '../client/LatticeClient';
import {
  ModelConfig,
  ModelDeployment,
  ModelInfo,
...
export interface UseLatticeClientOptions extends LatticeClientConfig {
  autoConnect?: boolean;
}

export interface UseLatticeClientReturn {
  client: LatticeClient | null;
  isConnected: boolean;
...
/**
 * Hook for managing Lattice client connection
 */
export function useLatticeClient(options: UseLatticeClientOptions): UseLatticeClientReturn {
  if (!React) {
    throw new Error('React is required to use Lattice React hooks');
  }

  const [client, setClient] = useState<LatticeClient | null>(null);
  ...
  const connect = useCallback(async () => {
    ...
    try {
      const newClient = new LatticeClient(options);
      setClient(newClient);
 ...
 */
export function useModelDeployment(client: LatticeClient | null): UseModelDeploymentReturn {
  if (!React) {
    throw new Error('React is required to use Lattice React hooks');
  }
 ...
 */
export function useInference(client: LatticeClient | null): UseInferenceReturn {
  if (!React) {
    throw new Error('React is required to use Lattice React hooks');
  }
