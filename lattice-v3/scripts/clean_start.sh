...
# Clean start script for Lattice v3
# Removes all state databases and build artifacts, then starts fresh

echo "🧹 Cleaning all Lattice state databases and build artifacts..."
...
rm -rf .lattice-mainnet/
rm -rf gui/lattice-core/.lattice-devnet/
...
if [ $? -eq 0 ]; then
    ...
    echo "  - GUI: cd gui/lattice-core && npm run tauri dev"
