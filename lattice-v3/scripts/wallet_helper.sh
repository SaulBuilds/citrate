...
WALLET_CLI="$LATTICE_DIR/target/debug/wallet"
RPC_URL="${RPC_URL:-http://localhost:8545}"
CHAIN_ID="${CHAIN_ID:-1337}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.lattice-wallet}"

# Utility functions
print_header() {
    echo -e "${BLUE}===============================================${NC}"
    echo -e "${BLUE}           Lattice Wallet Helper${NC}"
    echo -e "${BLUE}===============================================${NC}"
    echo
}
...
check_prerequisites() {
    ...
    if [ ! -f "$WALLET_CLI" ]; then
        ...
        echo "  Run: cargo build -p lattice-wallet --bin wallet"
        errors=$((errors + 1))
    else
    ...
    if curl -s -f "$RPC_URL" >/dev/null 2>&1; then
        ...
        echo "  Start node with: cargo run --bin lattice-node"
...
show_usage() {
    echo "Lattice Wallet Helper - Simplified wallet operations"
    echo
    ...
    echo "  KEYSTORE_DIR  - Keystore directory (default: ~/.lattice-wallet)"
    echo
    echo "Examples:"
