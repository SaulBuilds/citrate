...
impl BlockPropagation {
    ...
    pub async fn handle_new_block(
        ...
        if peers.contains(peer_id) {
            debug!("Already received block {} from peer {}", block_hash, peer_id);
            return Ok(());
        }
        ...
        // Cache the header
        self.header_cache.write().await.insert(block_hash, block.header.clone());
    ...
    async fn broadcast_block_except(
        &self,
        block: Block,
        except_peer: &PeerId,
    ) -> Result<()> {
        ...
        if !target_peers.is_empty() {
            ...
            debug!("Propagated block {} to {} peers", block_hash, target_peers.len());
        }
    ...
    pub async fn handle_blocks_response(
        &self,
        peer_id: &PeerId,
        blocks: Vec<Block>,
    ) -> Result<()> {
        ...
        for block in &blocks {
            self.downloading.write().await.remove(&block.header.block_hash);
            
            // Cache headers
            self.header_cache.write().await.insert(
...
mod tests {
    ...
    async fn test_block_propagation() {
        ...
        assert!(propagation.handle_new_block(&peer_id, block.clone()).await.is_ok());
        
        let cached = propagation.get_cached_header(&block.header.block_hash).await;
