...
impl ChainSelector {
    ...
    pub async fn on_new_block(&self, block: &Block) -> Result<bool, ChainSelectionError> {
        let new_blue_score = self.ghostdag
    ...
    async fn attempt_reorganization(&self, new_tip_block: &Block) -> Result<bool, ChainSelectionError> {
        let current_chain = self.current_chain.read().await;
        let old_tip = current_chain.tip;
        drop(current_chain);
        
        // Find common ancestor
        let (common_ancestor, reorg_depth) = self.find_common_ancestor(old_tip, new_tip_block.hash()).await?;
        ...
        let new_chain = self.build_chain(common_ancestor, new_tip_block.hash()).await?;
        
        // Perform reorganization
        self.perform_reorg(old_tip, new_tip_block.hash(), new_chain, reorg_depth).await?;
        
        Ok(true)
    }
    
    /// Find common ancestor between two chains
    async fn find_common_ancestor(&self, tip1: Hash, tip2: Hash) -> Result<(Hash, u64), ChainSelectionError> {
    ...
    async fn build_chain(&self, ancestor: Hash, tip: Hash) -> Result<Vec<Hash>, ChainSelectionError> {
    ...
    async fn perform_reorg(
        ...
        let new_tip_block = self.dag_store
            .get_block(&new_tip)
            .await
...
mod tests {
    ...
    async fn setup_test_env() -> (Arc<DagStore>, Arc<GhostDag>, Arc<TipSelector>, Arc<ChainSelector>) {
