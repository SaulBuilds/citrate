...
impl ProviderRegistry {
    ...
    pub async fn register_provider(&self, info: ProviderInfo) -> Result<()> {
        ...
        self.reputation_scores.write().await.insert(address, reputation);
        
        info!("Provider registered: {} with capacity: {} GB memory, {} compute units",
    ...
    pub async fn register_model_provider(
        ...
        debug!("Provider {} registered for model {:?}",
        ...
        for provider_addr in providers {
            if let Some(info) = provider_infos.get(provider_addr) {
                ...
                let score = self.calculate_provider_score(
                    &info.capacity,
                    reputation_scores.get(provider_addr),
                );
    ...
    pub async fn update_reputation(
        ...
        score.average_latency = (score.average_latency * (score.total_jobs - 1) + latency) / score.total_jobs;
    ...
    fn meets_requirements(
        ...
        if requirements.gpu_required {
            let has_gpu = capacity.hardware.iter().any(|h| {
                matches!(h, HardwareType::GPU(_))
            });
            if !has_gpu {
                return false;
            }
