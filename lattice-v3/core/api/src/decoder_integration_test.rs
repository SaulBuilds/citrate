// lattice-v3/core/api/src/decoder_integration_test.rs
...
mod tests {
    ...
    /// Test Lattice native transaction
    #[test]
    fn test_lattice_native_transaction() {
        let decoder = DecoderFactory::testing();

        // Create a valid Lattice transaction
        let lattice_tx = Transaction {
            hash: lattice_consensus::types::Hash::new([1u8; 32]),
            from: lattice_consensus::types::PublicKey::new([2u8; 32]),
            to: Some(lattice_consensus::types::PublicKey::new([3u8; 32])),
            value: 1000000000000000000, // 1 ETH
            data: vec![],
            nonce: 1,
            gas_price: 1000000000, // 1 gwei
            gas_limit: 21000,
            signature: lattice_consensus::types::Signature::new([4u8; 64]),
            tx_type: None,
        };
        ...
        match decoder.decode_eth_transaction(&tx_bytes) {
            Ok(decoded_tx) => {
                println!("Successfully decoded Lattice native transaction");
            ...
            Err(e) => {
                panic!("Failed to decode valid Lattice transaction: {}", e);
            }
        }
    }
