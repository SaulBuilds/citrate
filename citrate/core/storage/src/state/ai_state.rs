// citrate/core/storage/src/state/ai_state.rs

use anyhow::Result;
use citrate_consensus::types::Hash;
use citrate_execution::{Address, JobId, ModelId, ModelState, TrainingJob};
    ...
    pub fn calculate_training_root(&self) -> Result<Hash> {
        ...
        for (job_id, job) in job_entries {
            ...
            let status_byte = match job.status {
                citrate_execution::JobStatus::Pending => 0u8,
                citrate_execution::JobStatus::Active => 1u8,
                citrate_execution::JobStatus::Completed => 2u8,
                citrate_execution::JobStatus::Failed => 3u8,
                citrate_execution::JobStatus::Cancelled => 4u8,
            };
...
mod tests {
    use super::*;
    use citrate_execution::{AccessPolicy, ModelMetadata, UsageStats};
