...
export class LatticeClient {
    ...
    async connect(): Promise<void> {
        try {
            ...
            throw new Error(`Failed to connect to Lattice node: ${error}`);
        }
    }
    ...
    async deployModel(modelData: Buffer, config: DeploymentConfig): Promise<DeploymentResult> {
        try {
            ...
            const result = await this.rpcCall('lattice_deployModel', [{
    ...
    async runInference(modelId: string, inputData: any): Promise<InferenceResult> {
        try {
            const result = await this.rpcCall('lattice_runInference', [modelId, inputData]);
            return result;
        } catch (error: any) {
