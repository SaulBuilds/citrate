...
# Citrate v3 GUI Testnet Connection Script
...
build_gui_if_needed() {
    if [ ! -d "$PROJECT_ROOT/gui/citrate-core/node_modules" ]; then
        echo -e "${YELLOW}Installing GUI dependencies...${NC}"
        cd "$PROJECT_ROOT/gui/citrate-core"
        npm install
    fi
}
...
echo "   cd gui/citrate-core && npm run tauri dev"
echo ""
