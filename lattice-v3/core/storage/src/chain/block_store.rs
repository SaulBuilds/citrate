...
impl BlockStore {
    ...
    pub fn put_block(&self, block: &Block) -> Result<()> {
        ...
        // Store full block
        self.db.batch_put_cf(&mut batch, CF_BLOCKS, hash.as_bytes(), &block_bytes)?;
        
        // Store header separately for quick access
        let header_bytes = bincode::serialize(&block.header)?;
        self.db.batch_put_cf(&mut batch, CF_HEADERS, hash.as_bytes(), &header_bytes)?;
        
        // Store height -> hash mapping
        let height_key = height_to_key(block.header.height);
        self.db.batch_put_cf(&mut batch, CF_METADATA, &height_key, hash.as_bytes())?;
        ...
        for parent in block.parents() {
            ...
            self.db.batch_put_cf(&mut batch, CF_DAG_RELATIONS, &parent_children_key, &children_bytes)?;
        }
        
        // Store blue set information
        if block.header.blue_score > 0 {
            let blue_score_key = blue_score_key(block.header.blue_score);
            self.db.batch_put_cf(&mut batch, CF_BLUE_SET, &blue_score_key, hash.as_bytes())?;
        }
    ...
    pub fn delete_block(&self, hash: &Hash) -> Result<()> {
        ...
        if let Some(block) = self.get_block(hash)? {
            ...
            // Delete block
            self.db.batch_delete_cf(&mut batch, CF_BLOCKS, hash.as_bytes())?;
            
            // Delete header
            self.db.batch_delete_cf(&mut batch, CF_HEADERS, hash.as_bytes())?;
            
            // Delete height mapping
            let height_key = height_to_key(block.header.height);
            self.db.batch_delete_cf(&mut batch, CF_METADATA, &height_key)?;
            ...
            let blue_score_key = blue_score_key(block.header.blue_score);
            self.db.batch_delete_cf(&mut batch, CF_BLUE_SET, &blue_score_key)?;
