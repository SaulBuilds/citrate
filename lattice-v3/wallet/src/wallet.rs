...
impl Wallet {
    ...
    /// Create new account
    pub fn create_account(&mut self, password: &str, alias: Option<String>) -> Result<Account, WalletError> {
    ...
    pub fn import_account(&mut self, private_key_hex: &str, password: &str, alias: Option<String>) -> Result<Account, WalletError> {
        let verifying_key = self.keystore.import_key(private_key_hex, password, alias.clone())?;
    ...
    pub async fn send_transaction(
        ...
        // Get account
        let account = self.get_account(from_index)
    ...
    pub async fn get_transaction_receipt(&self, tx_hash: &Hash) -> Result<Option<serde_json::Value>, WalletError> {
