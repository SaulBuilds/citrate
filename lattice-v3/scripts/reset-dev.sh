...
rm_if_exists "$DATA_ROOT/lattice-core/accounts.json"

# GUI node config
rm_if_exists "$CONFIG_ROOT/lattice-core/config.json"

echo "[3/5] Removing GUI node data directories to force fresh genesis…"
# NodeManager default data_dir = data_dir()/"lattice" with subdir "chain"
rm_if_exists "$DATA_ROOT/lattice"

echo "[4/5] Removing standalone node data (if used)…"
rm_if_exists "$HOME/.lattice"

echo "[5/5] Clearing OS keychain entries for lattice-core wallets (best effort)…"
if command -v security >/dev/null 2>&1; then
  # macOS Keychain cleanup
  # Enumerate accounts for service "lattice-core" and delete wallet_* entries
  mapfile -t ACCOUNTS < <(security find-generic-password -s lattice-core 2>/dev/null | awk -F '="' '/acct/ {print $2}' | sed 's/"$//') || true
  if [[ ${#ACCOUNTS[@]} -gt 0 ]]; then
    for acct in "${ACCOUNTS[@]}"; do
      if [[ "$acct" == wallet_* ]]; then
        echo "- Deleting keychain entry: $acct"
        security delete-generic-password -s lattice-core -a "$acct" >/dev/null 2>&1 || true
      fi
    done
  else
    echo "- No keychain entries found for service lattice-core"
  fi
else
  echo "- 'security' CLI not found; if on Linux, clear secrets from your keyring manager (service=lattice-core, account=wallet_<address>)"
fi
