use crate::{NetworkError, NetworkMessage, peer::{Peer, PeerId, PeerManager}};
use dashmap::DashMap;
...
impl GossipProtocol {
    ...
    pub async fn handle_new_block(
        ...
        // Mark as seen
        self.seen_blocks.insert(hash, SeenItem {
            hash,
    ...
    pub async fn handle_new_transaction(
        ...
        // Mark as seen
        self.seen_transactions.insert(hash, SeenItem {
            hash,
            first_seen: Instant::now(),
            propagated: false,
        });
    ...
    async fn validate_block(&self, block: &Block) -> bool {
        ...
        let now = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_secs();
        if block.header.timestamp > now + 900 { // Allow 15 minutes clock drift
            return false;
        }
    ...
    pub async fn cleanup_seen_cache(&self) {
        ...
        // Clean blocks
        self.seen_blocks.retain(|_, item| {
            now.duration_since(item.first_seen) < ttl
        });
        
        // Clean transactions
        self.seen_transactions.retain(|_, item| {
            now.duration_since(item.first_seen) < ttl
        });
...
mod tests {
    ...
    async fn test_seen_cache() {
        ...
        // Mark as seen
        gossip.seen_blocks.insert(hash, SeenItem {
            hash,
            first_seen: Instant::now(),
            propagated: false,
        });
