...
# Lattice SDK Installation Script
# Installs Lattice SDKs for multiple programming languages
...
print_header() {
    echo ""
    echo -e "${BLUE}============================================${NC}"
    echo -e "${BLUE} Lattice AI Blockchain SDK Installer${NC}"
    echo -e "${BLUE}============================================${NC}"
    echo ""
}
...
install_nodejs_sdk() {
    ...
    if [[ -f "package.json" ]]; then
        log "Installing Lattice SDK in current project..."
        npm install @lattice-ai/sdk
    else
        log "Installing Lattice SDK globally..."
    ...
    echo ""
    echo "Usage example:"
    echo "  import { LatticeSDK } from '@lattice-ai/sdk';"
    echo "  const sdk = new LatticeSDK({ nodeUrl: 'http://localhost:8545' });"
}
...
install_python_sdk() {
    ...
    if [[ -n "$VIRTUAL_ENV" ]]; then
        ...
        $pip_cmd install lattice-sdk
    else
        log "Installing globally (consider using a virtual environment)"
        $pip_cmd install --user lattice-sdk
    fi

    log "Python SDK installed successfully! ✓"
    echo ""
    echo "Usage example:"
    echo "  from lattice_sdk import LatticeClient"
    echo "  client = LatticeClient(node_url='http://localhost:8545')"
}
...
install_cli_tools() {
    ...
    if [[ "$os" == "macos" ]] && check_command brew; then
        ...
        if brew tap lattice-ai/tap 2>/dev/null || true; then
    ...
    if [[ "$version" == "latest" ]]; then
        ...
        version=$(curl -s https://api.github.com/repos/lattice-ai/lattice-v3/releases/latest | grep -o '"tag_name": "[^"]*' | cut -d'"' -f4)
        if [[ -z "$version" ]]; then
            version="v0.1.0"  # fallback version
        fi
    fi

    local filename="lattice-${version#v}-${os}-${arch}.tar.gz"
    local url="https://github.com/lattice-ai/lattice-v3/releases/download/${version}/${filename}"
    ...
    # Copy binaries
    for binary in lattice lattice-cli lattice-wallet faucet; do
...
main() {
    ...
    log "Starting Lattice SDK installation..."
    echo ""
    ...
    echo "  2. Join our Discord: https://discord.gg/lattice-ai"
    echo "  3. Start building with Lattice!"
}
