...
use lattice_execution::executor::ArtifactService;
...
impl NodeArtifactService {
    pub fn new(api_base: Option<String>) -> Self {
        ...
        let apis = if let Ok(list) = std::env::var("LATTICE_IPFS_PROVIDERS") {
            list.split(',').map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).collect()
        } else {
            vec![api_base.unwrap_or_else(|| "http://127.0.0.1:5001".to_string())]
        };
        Self { client: reqwest::Client::new(), apis }
    }
...
impl ArtifactService for NodeArtifactService {
    async fn pin(&self, cid: &str, replicas: usize) -> Result<(), ExecutionError> {
        ...
        for base in &self.apis {
            ...
            if successes >= needed { return Ok(()); }
        }
        Err(ExecutionError::Reverted(last_err.unwrap_or_else(|| "pin failed".into())))
    }
            ...
            let status = match self.client.post(&url).send().await {
                Ok(resp) if resp.status().is_success() => {
                    match resp.text().await {
                        Ok(body) => if body.contains(cid) { "pinned" } else { "unpinned" },
                        Err(_) => "unknown",
                    }
                },
                _ => "unknown",
    ...
    async fn add(&self, data: &[u8]) -> Result<String, ExecutionError> {
        ...
        let base = self.apis.first().cloned().unwrap_or_else(|| "http://127.0.0.1:5001".to_string());
        ...
        if !resp.status().is_success() {
            return Err(ExecutionError::Reverted(format!("ipfs add status: {}", resp.status())));
        }
        let json: serde_json::Value = resp.json().await
            .map_err(|e| ExecutionError::Reverted(format!("ipfs add parse error: {}", e)))?;
        let cid = json["Hash"].as_str().unwrap_or("").to_string();
        if cid.is_empty() { return Err(ExecutionError::Reverted("ipfs add returned empty cid".into())); }
        Ok(cid)
    }
}
