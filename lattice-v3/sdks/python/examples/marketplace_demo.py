#!/usr/bin/env python3
"""
Marketplace demo for Lattice Python SDK
...
def main():
    ...
    # Model seller
    seller = LatticeClient(rpc_url=RPC_URL, private_key=KeyManager().get_private_key())
    print(f"Seller: {seller.key_manager.get_address()}")

    # Model buyer
    buyer = LatticeClient(rpc_url=RPC_URL, private_key=KeyManager().get_private_key())
    ...
    try:
        ...
        buyers = []
        for i in range(3):
            buyer_client = LatticeClient(rpc_url=RPC_URL, private_key=KeyManager().get_private_key())
