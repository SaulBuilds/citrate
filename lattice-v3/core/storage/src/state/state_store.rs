...
impl StateStore {
    ...
    pub fn put_model(&self, model_id: &ModelId, model: &ModelState) -> Result<()> {
        ...
        self.db.put_cf(CF_MODELS, model_id.0.as_bytes(), &model_bytes)?;
    ...
    pub fn put_training_job(&self, job: &TrainingJob) -> Result<()> {
        ...
        self.db.put_cf(CF_TRAINING, job.id.0.as_bytes(), &job_bytes)?;
    ...
    pub fn create_snapshot(&self, block_hash: &Hash, accounts: Vec<(Address, AccountState)>) -> Result<()> {
        let mut batch = self.db.batch();
        
        for (address, account) in accounts {
            let snapshot_key = snapshot_account_key(block_hash, &address);
            let account_bytes = bincode::serialize(&account)?;
            self.db.batch_put_cf(&mut batch, CF_STATE, &snapshot_key, &account_bytes)?;
        }
    ...
    pub fn get_snapshot_account(&self, block_hash: &Hash, address: &Address) -> Result<Option<AccountState>> {
