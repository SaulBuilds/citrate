use crate::types::{
    error::ApiError,
    request::{TransactionRequest, CallRequest},
};
...
impl TransactionApi {
    ...
    pub async fn estimate_gas(&self, request: CallRequest) -> Result<u64, ApiError> {
        ...
        let data_gas = request.data.as_ref().map_or(0, |d| {
            d.iter().map(|&byte| if byte == 0 { 4 } else { 68 }).sum::<u64>()
        });
    ...
    pub async fn get_transaction_count(&self, address: lattice_execution::types::Address) -> Result<u64, ApiError> {
