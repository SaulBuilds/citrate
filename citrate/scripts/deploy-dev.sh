...
# Citrate V3 Development Deployment Script
...
build_rust() {
    ...
    case $component in
        "node")
            log "Building Citrate node ($mode)..."
            cargo build $flag --bin lattice
            info "Node binary: $target_dir/lattice"
            ;;
        "cli")
            log "Building CLI tools ($mode)..."
            cargo build $flag --bin citrate-cli
            cargo build $flag --bin citrate-wallet
            cargo build $flag --bin faucet
            info "CLI binaries: $target_dir/{citrate-cli,citrate-wallet,faucet}"
            ;;
        "all")
...
start_dev_servers() {
    ...
    if [[ -f "target/debug/lattice" ]]; then
        ...
        ./target/debug/lattice --network devnet --data-dir .citrate-dev &
        echo $! > .citrate-dev.pid
        sleep 2
    fi
...
# Main function
main() {
    echo "🚀 Citrate V3 Development Deployment"
