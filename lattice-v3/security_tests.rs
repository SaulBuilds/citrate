...
mod security_tests {
    ...
    mod key_management_tests {
        ...
        fn test_access_policy_enforcement() {
            ...
            let mut policy = lattice_execution::crypto::key_manager::AccessPolicy {
                owner,
                full_access: vec![],
            ...
            assert!(manager.check_access(
                model_id, owner,
                lattice_execution::crypto::key_manager::AccessType::Full
            ).unwrap());

            // User should have inference access only
            assert!(manager.check_access(
                model_id, user,
                lattice_execution::crypto::key_manager::AccessType::Inference
            ).unwrap());
