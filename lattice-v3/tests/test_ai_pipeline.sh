...
echo -e "${GREEN}════════════════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}       Lattice AI Pipeline Test Suite${NC}"
echo -e "${GREEN}════════════════════════════════════════════════════════════════${NC}"
echo ""
...
check_prerequisites() {
    ...
    if ! curl -s -X POST $RPC_URL \
        ...
        echo -e "${YELLOW}⚠️  Starting Lattice devnet...${NC}"
...
test_list_models() {
    ...
    ./target/release/lattice-cli model list --limit 10
...
run_all_tests() {
    ...
    if [ ! -z "$LATTICE_PID" ]; then
        echo -e "${YELLOW}Stopping Lattice node...${NC}"
