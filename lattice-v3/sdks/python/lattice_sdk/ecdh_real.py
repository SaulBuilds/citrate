    ...
    def __init__(self, private_key_bytes: bytes = None):
        ...
        if private_key_bytes:
            ...
            if len(private_key_bytes) != 32:
                raise LatticeError("Private key must be 32 bytes")
        ...
        try:
            ...
            if len(peer_public_key_bytes) == 32:
                ...
                if peer_public_key_bytes[0] not in [0x02, 0x03]:
                    raise LatticeError("Invalid compressed public key prefix")
                ...
                if peer_public_key_bytes[0] != 0x04:
                    raise LatticeError("Invalid uncompressed public key prefix")
                ...
                raise LatticeError(f"Invalid public key length: {len(peer_public_key_bytes)}")
            ...
            raise LatticeError(f"ECDH key exchange failed: {str(e)}")

    def derive_shared_secret(self, peer_public_key_bytes: bytes,
                           salt: bytes = b"lattice-ecdh",
